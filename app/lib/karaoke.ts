...
export function alignRoughWordsToLyrics(
  ...
  const lines = visibleLyricLines(lyrics);
  const lyricTokens: Array<{ text: string; line: number }> = [];
  const lineTokenIndexes: number[][] = [];
  ...
  for (let i = 1; i <= n; i++) {
    ...
    for (let j = 1; j <= m; j++) {
      ...
      const match = prev[j - 1] + (2 * sim - 0.72);
      const skipLyric = prev[j] + gap;
      const skipAsr = cur[j - 1] + gap;
  ...
  const matches: Array<number | null> = new Array(n).fill(null);
  let i = n;
  let j = m;
  while (i > 0 || j > 0) {
    const step = back[i]?.[j] ?? 0;
    if (i > 0 && j > 0 && step === 0) {
      if (tokenSimilarity(lyricTokens[i - 1].text, asr[j - 1].text) >= 0.42) {
        matches[i - 1] = j - 1;
      }
      i--;
      j--;
  ...
  const timed: Array<KaraokeWord | null> = lyricTokens.map((token, index) => {
    const matched = matches[index];
    if (matched == null) return null;
    const word = asr[matched];
    return { text: token.text, start: word.start, end: Math.max(word.start + 0.01, word.end) };
  });

  const anchors = timed.map((value, index) => value ? index : -1).filter((index) => index >= 0);
  if (!anchors.length) return buildEstimatedKaraokeTimeline(lyrics, duration);

  const boundaries = [-1, ...anchors, timed.length];
  for (let b = 0; b < boundaries.length - 1; b++) {
    const left = boundaries[b];
    const right = boundaries[b + 1];
    const count = right - left - 1;
    if (count <= 0) continue;

    const leftTime = left >= 0 && timed[left] ? timed[left]!.end : 0;
    const rightTime = right < timed.length && timed[right] ? timed[right]!.start : duration;
    const available = Math.max(0.04 * count, rightTime - leftTime);
    const step = available / count;

    for (let k = 0; k < count; k++) {
      const index = left + 1 + k;
      const start = Math.max(0, leftTime + step * k);
      const end = Math.min(duration, Math.max(start + 0.02, leftTime + step * (k + 1)));
      timed[index] = { text: lyricTokens[index].text, start, end };
    }
  }

  const built = lines.map((text, lineIndex) => {
    const words = lineTokenIndexes[lineIndex]
      .map((index) => timed[index])
      .filter((word): word is KaraokeWord => Boolean(word));
