...
async function generateVideo(song: Song, karaoke = false, suppliedTimeline: KaraokeLine[] = []) {
  ...
  let audioBlob: Blob;
  try {
    audioBlob = await convertMedia(processedAudio, 'm4a');
  } catch {
    audioBlob = await convertMedia(processedAudio, 'mp3');
  }
...
async function convertMedia(source: Blob, format: 'mp3' | 'wav' | 'm4a') {
  ...
  if (format === 'm4a' && !(await canEncodeAudio('aac'))) {
    throw new Error('Trình duyệt không encode AAC; sẽ fallback sang MP3 cho video.');
  }
