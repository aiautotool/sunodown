  ...
  '@graph': [
    {
      ...
      featureList: ['Tải Suno MP3', 'Tải Suno WAV', 'Tải Suno M4A', 'Tải video Suno', 'Tải ảnh bìa', 'Tải lyrics', 'Tạo video visualizer'],
    },
    {
      '@type': 'HowTo',
      name: 'Cách tải nhạc Suno',
      step: [
...
export default function Page() {
  return (
    <>
      ...
      <section className="bg-[#080812] px-5 pt-12 text-white sm:px-8 sm:pt-16">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold text-white/55">
            {['MP3', 'WAV', 'M4A', 'Video', 'Ảnh bìa', 'Lyrics'].map((item) => <span key={item} className="rounded-full border border-white/10 bg-white/[.05] px-3 py-1.5">✓ {item}</span>)}
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#080812] px-5 pb-20 text-white sm:px-8">
        <div className="mx-auto max-w-5xl border-t border-white/10 pt-12">
          <div className="max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[.22em] text-violet-300">Tải nhạc Suno online</p>
            <h2 className="mt-3 text-3xl font-bold sm:text-4xl">Một link Suno, tải đủ file bạn cần</h2>
            <p className="mt-4 leading-7 text-white/65">Suno Grab ưu tiên chức năng tải xuống trước: audio, video, cover, lyrics và metadata. Visualizer là công cụ nâng cao để biến bài hát thành video đăng TikTok, Reels, Shorts hoặc YouTube.</p>
          </div>

          <div className="mt-10 grid gap-4 md:grid-cols-4">
            {[
              ['Tải Suno MP3/WAV', 'Chuyển audio sang MP3 hoặc WAV ngay trên trình duyệt.'],
              ['Tải video Suno', 'Lưu video gốc nếu bài hát có video.'],
              ['Cover + Lyrics', 'Lưu ảnh bìa và lời bài hát khi dữ liệu có sẵn.'],
              ['Visualizer Studio', 'Tạo video 9:16, 16:9, 1:1 với waveform và template.'],
            ].map(([title, text]) => <article key={title} className="rounded-2xl border border-white/10 bg-white/[.045] p-5"><h3 className="font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-white/55">{text}</p></article>)}
          </div>
          ...
          <div className="mt-12 grid gap-8 md:grid-cols-[.9fr_1.1fr]">
            <div>
              ...
              <ol className="mt-5 space-y-4 text-sm leading-6 text-white/65">
                ...
                <li><b className="text-white">2.</b> Dán link vào ô ở đầu trang. Công cụ tự nhận diện bài hát.</li>
                <li><b className="text-white">3.</b> Chọn MP3, WAV, M4A, video, cover, lyrics hoặc metadata.</li>
                <li><b className="text-white">4.</b> Nếu muốn đăng mạng xã hội, mở Visualizer để tạo video.</li>
              </ol>
            </div>
            <div>
              ...
              <div className="mt-5 space-y-3">{faq.map((item) => <details key={item.question} className="rounded-2xl border border-white/10 bg-white/[.04] p-4"><summary className="cursor-pointer font-semibold">{item.question}</summary><p className="mt-3 text-sm leading-6 text-white/60">{item.answer}</p></details>)}</div>
            </div>
          </div>
