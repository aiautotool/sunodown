...
async function dispatch(env:Env,id:string):Promise<'done'|'pending'>{const row=await env.RENDER_DB.prepare('SELECT input_json,status FROM render_jobs WHERE id=?').bind(id).first<{input_json:string;status:string}>();if(!row||row.status==='completed'||row.status==='failed')return'done';const input=JSON.parse(row.input_json),serviceUrl=input.mode==='ai_music_video'?(env.AI_RENDER_SERVICE_URL||env.RENDER_SERVICE_URL):env.RENDER_SERVICE_URL;if(!serviceUrl){await update(env,id,'failed',0,input.mode==='ai_music_video'?'Máy chủ AI Music Video chưa được cấu hình.':'Server renderer chưa được cấu hình.');return'done'}await update(env,id,'preparing',5);const response=await fetch(serviceUrl,{method:'POST',headers:{'content-type':'application/json',...(env.RENDER_SERVICE_TOKEN?{authorization:`Bearer ${env.RENDER_SERVICE_TOKEN}`}:{})},body:JSON.stringify({jobId:id,input})});if(response.status===202){await update(env,id,'rendering',20);return'pending'}if(!response.ok){const detail=(await response.text()).slice(0,500);throw new Error(`Renderer HTTP ${response.status}${detail?`: ${detail}`:''}`)}const contentType=response.headers.get('content-type')||'';if(contentType.includes('video/')){await update(env,id,'uploading',95);const key=`renders/${id}.mp4`;await env.RENDER_RESULTS.put(key,response.body,{httpMetadata:{contentType:'video/mp4',cacheControl:'public, max-age=604800'}});await update(env,id,'completed',100,null,`/api/render/results/${id}`,key);return'done'}const data=await response.json() as any;if(data.status==='failed'){await update(env,id,'failed',20,String(data.error||'Renderer thất bại'));return'done'}if(data.resultUrl){await update(env,id,'completed',100,null,String(data.resultUrl),data.resultKey||null);return'done'}throw new Error(data.error||'Renderer không trả về video')}
...
export default {fetch:safeFetch,async queue(batch:MessageBatch<Msg>,env:Env){for(const message of batch.messages){try{const state=await dispatch(env,message.body.jobId);if(state==='pending')await env.RENDER_QUEUE.send(message.body,{delaySeconds:30});message.ack()}catch(error){console.error('render dispatch',error);await update(env,message.body.jobId,'rendering',20,error instanceof Error?error.message:'Render failed');message.retry({delaySeconds:30})}}}} satisfies ExportedHandler<Env,Msg>;
