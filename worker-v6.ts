...
type Env={RENDER_DB:D1Database;RENDER_RESULTS:R2Bucket;RENDER_SERVICE_URL?:string;RENDER_SERVICE_TOKEN?:string};
...
async function safeFetch(request:Request,env:Env,ctx:ExecutionContext){
  try{return await app.fetch(request,env,ctx)}catch(error){
    ...
    if(request.method==='GET'&&!url.pathname.startsWith('/api/')){
      const assetEnv=env as Env&{ASSETS?:{fetch:(request:Request)=>Promise<Response>}};
      if(assetEnv.ASSETS){const asset=await assetEnv.ASSETS.fetch(request);if(asset.status!==404)return asset;const index=await assetEnv.ASSETS.fetch(new Request(new URL('/',url),request));if(index.status<500)return index}
    }
