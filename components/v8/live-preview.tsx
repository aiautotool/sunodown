...
export function LivePreview(props: Props) {
  ...
  return <div className="mb-4 rounded-xl border border-cyan-300/20 bg-black/30 p-3">
    ...
    <div className="flex justify-center overflow-hidden rounded-lg bg-black">
      {props.resultUrl
        ...
        : <div className="relative" style={{aspectRatio:`${size.width}/${size.height}`,width:`min(100%, ${340*size.width/size.height}px)`,maxHeight:340}}>
            ...
            {props.layout&&props.onLayoutChange&&(['wave','subtitle'] as const).map(key=>{if(key==='subtitle'&&props.lyrics==='off')return null;const p=props.layout![key],isWave=key==='wave',ww=isWave?Math.max(24,64*p.scale/100):Math.max(34,72*p.scale/100),hh=isWave?Math.max(8,18*p.scale/100):Math.max(10,18*p.scale/100);return <div key={key} role="button" tabIndex={0} aria-label={isWave?'Kéo trực tiếp vùng sóng nhạc':'Kéo trực tiếp subtitle'} title={isWave?'Nắm trực tiếp sóng để kéo':'Nắm ngay trên chữ subtitle để kéo'} onPointerDown={e=>{e.preventDefault();e.stopPropagation();const el=e.currentTarget,box=el.parentElement!.getBoundingClientRect(),startX=e.clientX,startY=e.clientY,origin={...p};el.setPointerCapture(e.pointerId);const move=(ev:PointerEvent)=>{const x=Math.max(0,Math.min(100,origin.x+(ev.clientX-startX)/box.width*100)),y=Math.max(0,Math.min(100,origin.y+(ev.clientY-startY)/box.height*100));props.onLayoutChange?.({...props.layout!,[key]:{...origin,x,y}})};const up=()=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up)};window.addEventListener('pointermove',move);window.addEventListener('pointerup',up,{once:true})}} className="absolute z-20 -translate-x-1/2 cursor-grab active:cursor-grabbing" style={{left:`${p.x}%`,top:isWave?`calc(${p.y}% - ${hh/2}%)`:`calc(${p.y}% - ${(58/100)*hh}%)`,width:`${ww}%`,height:`${hh}%`,touchAction:'none'}}><span className="pointer-events-none absolute inset-0 rounded-md border border-transparent transition-colors hover:border-white/25"/></div>})};const up=()=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up)};window.addEventListener('pointermove',move);window.addEventListener('pointerup',up,{once:true})}} className="absolute z-20 -translate-x-1/2 -translate-y-1/2 cursor-grab active:cursor-grabbing" style={{left:`${p.x}%`,top:`${p.y}%`,width:`${ww}%`,height:`${hh}%`,touchAction:'none'}}><span className="pointer-events-none absolute inset-0 rounded-md border border-transparent transition-colors hover:border-white/25"/></div>})}
          </div>}
    </div>
    ...
    {!props.resultUrl && <p className="mt-2 text-[11px] leading-5 text-white/45">{props.exporting ? 'Preview đã dừng trong lúc xuất video.' : 'Preview dùng audio thật làm clock. Nắm trực tiếp sóng hoặc subtitle trên video để kéo tới vị trí mong muốn.'}</p>}
  </div>;
}
