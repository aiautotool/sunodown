...
export function V8HistoryEnhancer(){
  ...
  useEffect(()=>{
    ...
    const capture=(event:Event)=>{
      ...
      const next={id:`link:${url}`,url,title:'Liên kết Suno',createdAt:Date.now()};
  ...
  return <section id="library-history" className="mt-5 rounded-2xl border border-white/10 bg-[#081421] p-4 sm:p-5">
    <div className="flex items-center justify-between gap-3">
      <div><p className="text-[10px] font-bold uppercase tracking-widest text-violet-300">Thư viện</p><h2 className="mt-1 font-bold">Lịch sử liên kết đã dùng</h2><p className="mt-1 text-xs text-white/40">Lưu trên thiết bị này, tối đa 50 mục.</p></div>
