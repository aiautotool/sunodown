...
export function PasteLinkEnhancer() {
  useEffect(() => {
    ...
    const pasteFromClipboard = async (showFeedback = true) => {
      ...
      try {
        ...
        if (showFeedback) button.textContent = 'Giữ để dán';
        input.focus();
        if (showFeedback) window.setTimeout(() => { button.textContent = original; }, 1800);
      }
    };

    const pasteButtonClick = () => { void pasteFromClipboard(true); };
    const pasteWhenInputClicked = () => {
      if (!input.value.trim()) void pasteFromClipboard(false);
    };

    button.addEventListener('click', pasteButtonClick);
    input.addEventListener('click', pasteWhenInputClicked);
    label.appendChild(button);

    return () => {
      button.removeEventListener('click', pasteButtonClick);
      input.removeEventListener('click', pasteWhenInputClicked);
      button.remove();
    };
  }, []);
