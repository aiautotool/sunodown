...
function drawProTemplate(ctx:CanvasRenderingContext2D,bmp:ImageBitmap,song:Song,w:number,h:number,t:number,level:number,template:VisualTemplate,p:Palette){
 ...
 if(template==='editorial'){
  ctx.fillStyle='#101714';ctx.fillRect(0,0,w,h);
...
function drawTemplate(ctx:CanvasRenderingContext2D,bmp:ImageBitmap,song:Song,w:number,h:number,t:number,level:number,template:VisualTemplate,motion:MotionIntensity,p:Palette){if(['editorial','spotlight','gold-record'].includes(template)){drawProTemplate(ctx,bmp,song,w,h,t*MOTION_GAIN[motion],level,template,p);return}const gain=MOTION_GAIN[motion];ctx.fillStyle='#080812';ctx.fillRect(0,0,w,h);if(template==='vinyl'){coverFill(ctx,bmp,w,h,.34,42,1.08+level*.03*gain);const portrait=h>w,cx=portrait?w/2:w*.36,cy=portrait?h*.39:h*.46,r=Math.min(w,h)*(portrait?.28:.29)*(1+level*.015*gain);ctx.save();ctx.translate(cx,cy);ctx.rotate(t*(.24+.16*gain));ctx.shadowColor=rgb(p[1],.35+.25*level);ctx.shadowBlur=22+level*26;ctx.fillStyle='#111116';ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);ctx.fill();ctx.save();ctx.beginPath();ctx.arc(0,0,r*.58,0,Math.PI*2);ctx.clip();const s=Math.max((r*1.16)/bmp.width,(r*1.16)/bmp.height),iw=bmp.width*s,ih=bmp.height*s;ctx.drawImage(bmp,-iw/2,-ih/2,iw,ih);ctx.restore();ctx.fillStyle='#0b0b10';ctx.beginPath();ctx.arc(0,0,r*.08,0,Math.PI*2);ctx.fill();ctx.restore();drawMeta(ctx,song,w,portrait?h*.72:h*.26,portrait?'center':'left',portrait?.8:.48,template,p);return}if(template==='glass-card'){coverFill(ctx,bmp,w,h,.52,46,1.12+level*.035*gain,Math.sin(t*.2)*w*.025*gain,Math.cos(t*.17)*h*.018*gain);const portrait=h>w,cw=w*(portrait?.78:.72),ch=h*(portrait?.54:.62),x=(w-cw)/2,y=h*(portrait?.17:.13);ctx.beginPath();ctx.roundRect(x,y,cw,ch,Math.max(22,w*.025));ctx.fillStyle='rgba(12,12,28,.52)';ctx.fill();ctx.strokeStyle=rgb(p[1],.42);ctx.stroke();ctx.save();ctx.beginPath();ctx.roundRect(x+cw*.08,y+ch*.08,cw*.84,ch*(portrait?.56:.7),Math.max(16,w*.018));ctx.clip();const aw=cw*.84,ah=ch*(portrait?.56:.7),s=Math.min(aw/bmp.width,ah/bmp.height),iw=bmp.width*s,ih=bmp.height*s;ctx.drawImage(bmp,x+cw*.08+(aw-iw)/2,y+ch*.08+(ah-ih)/2,iw,ih);ctx.restore();drawMeta(ctx,song,w,y+ch*(portrait?.72:.83),'center',.62,template,p);return}if(template==='lyrics-focus'){coverFill(ctx,bmp,w,h,.32,48,1.1+level*.03*gain);const r=Math.min(w,h)*(.11+level*.018*gain);ctx.save();ctx.beginPath();ctx.arc(w/2,h*.27,r,0,Math.PI*2);ctx.clip();coverFit(ctx,bmp,r*2,r*2,1);ctx.restore();drawMeta(ctx,song,w,h*.41,'center',.72,template,p);return}const zoom=1.06+.025*Math.sin(t*.45)*gain+level*.025*gain,dx=Math.sin(t*.22)*w*.018*gain,dy=Math.cos(t*.18)*h*.014*gain;coverFill(ctx,bmp,w,h,.5,34,zoom,dx,dy);ctx.fillStyle='rgba(3,4,12,.25)';ctx.fillRect(0,0,w,h);coverFit(ctx,bmp,w*.82,h*.72,1+.02*Math.sin(t*.5)*gain+level*.01*gain,dx*.25,dy*.25);drawMeta(ctx,song,w,h*.67,'center',.72,template,p)}

export function createLiveFramePainter(bitmap: ImageBitmap) {
 const palette = extractPalette(bitmap);
 return (ctx: CanvasRenderingContext2D, song: Song, w: number, h: number, time: number, template: VisualTemplate, wave: WaveStyle, motion: MotionIntensity, lyrics: LyricsMode) => {
  drawTemplate(ctx, bitmap, song, w, h, time, amplitude(null, 0, time), template, motion, palette);
