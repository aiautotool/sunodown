...
export function MasteringPanel({audio,title}:{audio:string;title:string}){
 ...
 const run=async()=>{setBusy(true);setError('');setProgress('Analyzing source…');try{const r=await fetch(audio,{cache:'no-store'});if(!r.ok)throw Error('Không tải được audio nguồn.');setProgress('EQ · dynamics · soft clip · Auto Guard…');let result=await masterAudio(await r.blob(),profile);let output=result.blob;if(spatial){setProgress('Creating 5D stereo space…');output=await render5DAudio(output,spatialAmount/100,spatialMode);}if(mastered)URL.revokeObjectURL(mastered);setBlob(output);setMastered(URL.createObjectURL(output));setMetrics(result.metrics);setProgress('')}catch(e){setError(e instanceof Error?e.message:'Mastering thất bại.')}finally{setBusy(false)}};
 ...
 return <section className="sd-master">
  ...
  <div className="sd-5d">
    <button className={spatial?'active':''} onClick={()=>setSpatial(v=>!v)}><Waves/><span><b>5D Audio</b><small>{spatial?'ON · spatial stereo':'OFF · original stereo image'}</small></span><i/></button>
    {spatial&&<div className="sd-5d-controls"><div>{(['wide','immersive','orbit'] as Spatial5DMode[]).map(x=><button key={x} className={spatialMode===x?'active':''} onClick={()=>setSpatialMode(x)}>{x==='wide'?'Wide':x==='immersive'?'Immersive':'Orbit'}</button>)}</div><label>Depth <input type="range" min="20" max="100" value={spatialAmount} onChange={e=>setSpatialAmount(+e.target.value)}/><span>{spatialAmount}%</span></label><small>Mid/side-style widening + short Haas delays. Bass remains centered; use headphones for the clearest effect.</small></div>}
  </div>
