...
export function MasteringPanel({audio,binary,title,value,onChange}:{audio:string;binary?:Blob|null;title:string;value?:import('@/components/presets/production-preset').ProductionMasteringConfig;onChange?:(value:import('@/components/presets/production-preset').ProductionMasteringConfig)=>void}){
 const [profile,setProfile]=useState<MasterProfileId>(value?.profile && value.profile !== 'original' ? value.profile : 'clean'),[audioSection,setAudioSection]=useState<'master'|'eq'|'5d'>('master'),[advancedOpen,setAdvancedOpen]=useState(false),[eqOpen,setEqOpen]=useState(false),[custom,setCustom]=useState<AdvancedMasterSettings|null>(value?.advanced||null),[spatial,setSpatial]=useState(value?.spatial?.enabled||false),[spatialMode,setSpatialMode]=useState<Spatial5DMode>(value?.spatial?.mode||'immersive'),[spatialAmount,setSpatialAmount]=useState(value?.spatial?.amount||65),[busy,setBusy]=useState(false),[progress,setProgress]=useState(''),[metrics,setMetrics]=useState<MasterMetrics|null>(null),[mastered,setMastered]=useState<string|null>(null),[blob,setBlob]=useState<Blob|null>(null),[error,setError]=useState('');
 useEffect(()=>()=>{if(mastered)URL.revokeObjectURL(mastered)},[mastered]);
 useEffect(()=>{if(!value)return;if(value.profile!=='original')setProfile(value.profile);setCustom(value.advanced||null);setSpatial(Boolean(value.spatial?.enabled));setSpatialMode(value.spatial?.mode||'immersive');setSpatialAmount(value.spatial?.amount||65)},[value]);
 ...
 const emit=(nextProfile:MasterProfileId=profile,nextCustom:AdvancedMasterSettings|null=custom,nextSpatial=spatial,nextMode=spatialMode,nextAmount=spatialAmount)=>onChange?.({profile:nextProfile,advanced:nextCustom||undefined,spatial:{enabled:nextSpatial,mode:nextMode,amount:nextAmount}});
 const changeSpatial=(v:boolean)=>{setSpatial(v);livePreview(v,spatialMode,spatialAmount);emit(profile,custom,v,spatialMode,spatialAmount)};
 const changeMode=(v:Spatial5DMode)=>{setSpatialMode(v);livePreview(spatial,v,spatialAmount);emit(profile,custom,spatial,v,spatialAmount)};
 const changeDepth=(v:number)=>{setSpatialAmount(v);livePreview(spatial,spatialMode,v);emit(profile,custom,spatial,spatialMode,v)};
 ...
 const settings=custom||defaults(),change=(patch:Partial<AdvancedMasterSettings>)=>{const next={...settings,...patch};setCustom(next);emit(profile,next)},changeBand=(index:number,patch:Partial<EqBand>)=>{const next={...settings,eqBands:settings.eqBands.map((b,i)=>i===index?{...b,...patch}:b)};setCustom(next);emit(profile,next)},resetAdvanced=()=>{setCustom(null);emit(profile,null)};
 ...
 return <section className="sd-master">
  ...
  <div className="sd-audio-subnav"><button className={audioSection==='master'?'active':''} onClick={()=>setAudioSection('master')}>Mastering</button><button className={eqOpen?'active':''} onClick={()=>setEqOpen(true)}>EQ</button><button className={audioSection==='5d'?'active':''} onClick={()=>setAudioSection('5d')}>5D</button></div>
  {audioSection==='master'&&<><StudioSegmented value={profile} options={MASTER_PROFILES.map(x=>({value:x.id,label:x.label}))} onChange={v=>{setProfile(v);setMetrics(null);previewProfile(v);emit(v,custom)}}/>
  ...
  <StudioModal open={eqOpen} title="Parametric EQ" description="8-band Precision EQ · Frequency / Gain / Q" onClose={()=>setEqOpen(false)} footer={<><StudioButton onClick={resetAdvanced} disabled={!custom}>Reset EQ</StudioButton><StudioButton tone="accent" onClick={()=>setEqOpen(false)}>Xong</StudioButton></>}><div className="sd-eq-full"><StudioPanel className="sd-eq-workspace"><div className="sd-section-kicker">ADVANCED EQ</div>      <div className="sd-channel-eq"><div className="sd-eq-title"><div><b>8-Band Parametric EQ</b><small>Precision EQ · ±12 dB · Frequency / Gain / Q</small></div><StudioBadge>STEREO EQ</StudioBadge></div>
      ...
      </div><StudioButton onClick={resetAdvanced} disabled={!custom}>Reset EQ về preset</StudioButton></StudioPanel></div></StudioModal>
